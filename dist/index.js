...
const GridNodeContext = React.createContext(null);
...
function useIntersectionObserver({ root, rootMargin, target, isEnabled, }) {
    ...
    React.useEffect(() => {
        if (isEnabled) {
            ...
            }, Object.assign({ root }, (rootMargin ? { rootMargin } : {})));
            if (target.current) {
...
function ColumnIntersectionObserver({ columnIndex, dataLength, onIsIntersectingChange, isEnabled, }) {
    ...
    return (React.createElement("div", { ref: intersectionRef, style: {
            backgroundColor: 'yellow',
            gridColumn: columnIndex + 1,
            gridRow: dataLength + 2,
        } }));
}
function useColumnsIntersectionObserver({ columns, dataLength, isEnabled, }) {
    const [columnVisibility, setColumnVisibility] = React.useState([
        ...Array(columns.length).fill(false),
    ]);
    const handleColumnIntersectingChange = React.useCallback((columnIndex, isIntersecting) => {
    ...
    return [
        columnVisibility,
        React.createElement(React.Fragment, null, columnVisibility.map((_c, columnIndex) => (React.createElement(ColumnIntersectionObserver, { columnIndex: columnIndex, dataLength: dataLength, isEnabled: isEnabled, key: columns[columnIndex].key, onIsIntersectingChange: handleColumnIntersectingChange })))),
    ];
}
...
function useDragDrop({ columns, defaultColumnMinWidth, isColumnDragDisabled, onColumnsOrderChange, }) {
    ...
    const headerCellRefs = React.useRef(new Map());
    const trackingCellRefs = React.useRef(new Map());
    const handleDragEnd = React.useCallback(({ source, destination }) => {
        setDraggingKey(null);
        ...
        if (destination) {
            ...
            if (onColumnsOrderChange &&
                ...
                sourceColumn &&
                destinationColumn) {
                onColumnsOrderChange({ index: source.index, key: sourceColumn.key }, { index: destination.index, key: destinationColumn.key });
            }
        }
    ...
    const handleDragUpdate = React.useCallback(({ source, destination }) => {
        if (isColumnDragDisabled) {
            return;
        }
        if (destination && gridNode) {
            const newColumnOrder = spliceColumnOrder(columnOrder, source.index, destination.index);
            const sortedColumns = sortColumns(columns, newColumnOrder);
            if (gridNode) {
                gridNode.style.gridTemplateColumns = columnsToGridTemplate(sortedColumns, defaultColumnMinWidth);
            }
    ...
    const handleDragStart = React.useCallback(({ source }) => {
        if (isColumnDragDisabled) {
            return;
        }
...
function usePaneScrollState() {
    ...
    const checkScroll = React.useCallback(() => {
        if (gridNode) {
            ...
            const nextCanScrollRight = canScroll && scrollWidth - scrollLeft > clientWidth + 4;
...
function applyColumnWidthDefaults(column) {
    ...
    if (isNumber(defaultWidth)) {
        ...
        return Object.assign({}, column, { width: defaultWidth });
    }
    return column;
}
...
function applyColumnMaxWidth(column) {
    const { maxWidth, width } = column;
    if (isNumber(maxWidth)) {
        return Object.assign({}, column, { width: Math.min(maxWidth, width) });
    }
    return column;
}
// assumes applyColumnWithDefaults has been called first and all widths are exactly specified
function applyColumnMinWidth(column) {
    ...
    if (isNumber(minWidth)) {
        return Object.assign({}, column, { width: Math.max(minWidth, width) });
    }
    return column;
}
...
function css(styles) {
  ...
  if (isFunction$1(styles) || isPlainObject(styles)) {
    // $FlowFixMe
    return flatten$1(interleave(EMPTY_ARRAY, [styles].concat(interpolations)));
  }

  // $FlowFixMe
  return flatten$1(interleave(styles, interpolations));
}
...
var ComponentStyle = function () {
  ...
  ComponentStyle.prototype.generateAndInjectStyles = function generateAndInjectStyles(executionContext, styleSheet) {
    ...
    var flatCSS = flatten$1(this.rules, executionContext, styleSheet);
...
function ResizeDragHandle({ isHidden, maxWidth = 1000, minWidth = 50, onWidthChange, onWidthChangeEnd, shouldMoveLeftToAvoidOverflow, targetRef, }) {
    ...
    return (React.createElement("div", { ref: ownRef, className: classnames(styles.ResizeDragHandle, {
            [styles.moveLeftToAvoidOverflow]: shouldMoveLeftToAvoidOverflow,
            [styles.isHidden]: isHidden,
        }) },
        React.createElement("div", { className: styles.ExtraHandle })));
}
...
const HeaderCellContainer = styled.div `
  ...
  ${({ isAnyDragging, notResizable }) => isAnyDragging && !notResizable ? isAnyDraggingStyles : ''}

  ${({ notResizable }) => (notResizable ? notResizableStyles : '')}
`;
const HeaderCellWrapped = ({ column: { key, width, label, minWidth, name, notResizable }, dragHandleProps, draggableInnerRef, draggableProps, draggingKey, headerCellRefs, isLastChild, onWidthChange, onWidthChangeEnd, style, transform, }) => {
    const ref = React.useRef(null);
    const isAnyDragging = Boolean(draggingKey);
    const handleWidthChange = React.useCallback((newWidth) => {
        if (onWidthChangeEnd) {
            onWidthChangeEnd(key, newWidth);
        }
    }, [key, onWidthChangeEnd]);
    const handleTransientWidthChange = React.useCallback((newWidth) => {
        onWidthChange(key, newWidth);
    }, [key, onWidthChange]);
    const displayLabel = isUndefined(label) ? name : label;
    return (React.createElement(HeaderCellContainer, Object.assign({ isDragging: key === draggingKey, isAnyDragging: isAnyDragging, notResizable: notResizable, ref: (node) => {
        ...
        } }, draggableProps, dragHandleProps, { style: Object.assign({}, style, { maxWidth: width, minWidth: width, transform,
            width }), title: key }),
        React.createElement(LabelContainer, null,
            React.createElement(Label, null, displayLabel)),
        onWidthChangeEnd && !notResizable && (React.createElement(ResizeDragHandle, { isHidden: isAnyDragging, minWidth: minWidth, shouldMoveLeftToAvoidOverflow: isLastChild, targetRef: ref, onWidthChange: handleTransientWidthChange, onWidthChangeEnd: handleWidthChange }))));
};
...
function HeaderCell(props) {
    ...
    if (notDraggable) {
        return React.createElement(HeaderCellWrapped, Object.assign({}, wrappedProps));
    }
    return (React.createElement(reactBeautifulDnd.Draggable, { key: key, index: columnIndex, draggableId: key, isDragDisabled: isDragDisabled }, ({ draggableProps, dragHandleProps, innerRef: draggableInnerRef }) => {
        const { style } = draggableProps;
        const transform = getAxisLockedTransform(style);
        return (React.createElement(HeaderCellWrapped, Object.assign({}, wrappedProps, { dragHandleProps: dragHandleProps, draggableInnerRef: draggableInnerRef, draggableProps: draggableProps, style: style, transform: transform })));
    }));
}
...
var styles$1 = {"Header":"Header_Header__1I5CE","isAnyDragging":"Header_isAnyDragging__1wGte"};
styleInject(css$2);
...
function Header({ columns: rawColumns, defaultColumnMinWidth, draggingKey, headerCellRefs, isColumnDragDisabled, onColumnWidthChange, trackingCellRefs, }) {
    ...
    const columns = rawColumns.map((column) => {
        ...
        return Object.assign({}, column, { width: trackingCell.clientWidth });
    });
    const trackCells = useTrackCells({
        headerCellRefs,
        trackingCellRefs,
    ...
    const handleColumnWidthChange = React.useCallback((key, newWidth) => {
        if (gridNode) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: classnames(styles$1.Header, {
                [styles$1.isAnyDragging]: Boolean(draggingKey),
            }) }, columns.map((column, i) => (React.createElement(HeaderCell, { key: column.key, column: column, columnIndex: i, draggingKey: draggingKey, isDragDisabled: isColumnDragDisabled, isLastChild: i === columns.length - 1, onWidthChange: handleColumnWidthChange, onWidthChangeEnd: onColumnWidthChange, headerCellRefs: headerCellRefs })))),
        columns.map(({ key }) => (React.createElement("div", { key: key, style: { gridRowStart: -2, gridRowEnd: -1 }, ref: (node) => {
                if (node) {
...
var styles$2 = {"Cell":"Cell_Cell__1-3zV","isLastRow":"Cell_isLastRow__1vtWp","Content":"Cell_Content__sRZrE","isDragging":"Cell_isDragging__1GXEv"};
styleInject(css$3);

const Cell = React.memo(React.forwardRef(({ cellComponent, column, columnIndex, datum, isColumnVisible, isDragging, isLastRow, rowIndex, }, ref) => {
    ...
    const className = classnames(styles$2.Cell, {
        [styles$2.isDragging]: isDragging,
        [styles$2.isLastRow]: isLastRow,
    });
    const style = {
        gridColumn: columnIndex + 1,
        gridRow: rowIndex + 2,
    };
    const children = React.createElement("div", { className: styles$2.Content }, value);
    if (cellComponent) {
        return React.createElement(React.Fragment, null, cellComponent({ children, className, style }));
    }
...
var styles$3 = {"RowOverlay":"RowOverlay_RowOverlay__2YnzW"};
styleInject(css$4);
...
function RowOverlay({ children, columnsLength, isHoveringRow, rowIndex, }) {
    ...
    return (React.createElement("div", { className: styles$3.RowOverlay, ref: ref, style: {
...
var styles$4 = {"Cell":"Row_Cell__Wkai2","isLastRow":"Row_isLastRow__kcUhG","Content":"Row_Content__2erQp","isDragging":"Row_isDragging__2r8Gs","Row":"Row_Row__3uXq8","isClickable":"Row_isClickable__3EDnY","NullCell":"Row_NullCell__2XBZD"};
styleInject(css$5);

const Row = React.memo(({ cellComponent, cellRefs, columns, columnVisibility, datum, draggingKey, isLastRow, isSelected, onClick, rowIndex, rowOverlay, }) => {
    ...
    return (React.createElement("div", Object.assign({}, hoverBind, { className: classnames(styles$4.Row, {
            [styles$4.isSelected]: isSelected,
            [styles$4.isClickable]: Boolean(onClick),
        }), onClick: handleRowClick, role: "row", tabIndex: 0, onKeyDown: handleKeyDown }),
        columns.map((column, columnIndex) => {
            const { key } = column;
            return (React.createElement(Cell, { cellComponent: cellComponent, column: column, datum: datum, isColumnVisible: Boolean(!columnVisibility || columnVisibility[columnIndex]), isDragging: key === draggingKey, isLastRow: isLastRow, key: key, ref: (node) => {
        ...
        rowOverlay && (React.createElement(RowOverlay, { columnsLength: columns.length, isHoveringRow: isHovering, rowIndex: rowIndex }, rowOverlay({
            datum,
...
var styles$5 = {"GridPane":"Grid_GridPane__3SgbV","Grid":"Grid_Grid__2Cmk2","HeaderCell":"Grid_HeaderCell__1MH3g","canScrollRight":"Grid_canScrollRight__25Eu4","canScrollLeft":"Grid_canScrollLeft__3SlfR"};
styleInject(css$6);

const GridWrapped = React.forwardRef(({ cellComponent, columns: rawColumns, data, defaultColumnMinWidth = 200, gridRef, onColumnsOrderChange, onColumnWidthChange, onRowClick, rowKey, rowOverlay, selectedRowIndexes, virtualizationEnabled, }, externalRef) => {
    const columns = React.useMemo(() => rawColumns
        .filter(({ hidden }) => !hidden)
        .map(applyColumnWidthDefaults)
        .map(applyColumnMinWidth)
        .map(applyColumnMaxWidth), [rawColumns]);
    const isColumnDragDisabled = !onColumnsOrderChange;
    const { canScrollLeft, canScrollRight } = usePaneScrollState();
    const { cellRefs, draggingKey, headerCellRefs, onDragEnd, onDragStart, onDragUpdate, trackingCellRefs, } = useDragDrop({
        columns,
        defaultColumnMinWidth,
        isColumnDragDisabled,
        onColumnsOrderChange,
    });
    const [columnVisibility, observerComponent,] = useColumnsIntersectionObserver({
        columns,
        dataLength: data.length,
        isEnabled: Boolean(virtualizationEnabled),
    });
    const rowKeyAccessor = React.useCallback((d) => (isString(rowKey) ? d[rowKey] : rowKey(d)), [rowKey]);
    return (React.createElement("div", { className: styles$5.GridPane },
        React.createElement(reactBeautifulDnd.DragDropContext, { onDragEnd: onDragEnd, onDragUpdate: onDragUpdate, onDragStart: onDragStart },
            React.createElement(reactBeautifulDnd.Droppable, { droppableId: "droppable", direction: "horizontal" }, ({ innerRef: droppableInnerRef, droppableProps }) => (React.createElement("div", Object.assign({ className: classnames(styles$5.Grid, {
                    [styles$5.canScrollLeft]: canScrollLeft,
                    [styles$5.canScrollRight]: canScrollRight,
                }), ref: (node) => {
                ...
                React.createElement(Header, { cellRefs: cellRefs, columns: columns, defaultColumnMinWidth: defaultColumnMinWidth, draggingKey: draggingKey, headerCellRefs: headerCellRefs, trackingCellRefs: trackingCellRefs, onColumnWidthChange: onColumnWidthChange, isColumnDragDisabled: isColumnDragDisabled }),
                data.map((datum, rowIndex) => (React.createElement(Row, { cellComponent: cellComponent, cellRefs: cellRefs, columns: columns, columnVisibility: columnVisibility, datum: datum, draggingKey: draggingKey, isLastRow: rowIndex === data.length - 1, isSelected: Boolean(selectedRowIndexes && selectedRowIndexes.has(rowIndex)), key: rowKeyAccessor(datum), onClick: onRowClick, rowIndex: rowIndex, rowOverlay: rowOverlay }))),
                observerComponent))))));
});
...
function useColumnOrderState(columns) {
    ...
    const onColumnOrderChange = React.useCallback((source, destination) => {
